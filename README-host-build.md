...
Guest-RAM access is controlled separately by the CMake option `NO_PAGING`:

- `NO_PAGING=ON` — disable paging and use direct QSPI PSRAM guest memory;
- `NO_PAGING=OFF` — enable the paging path implemented in `src/ega128_paging.c`.

`NO_PAGING` currently defaults to `ON`. CMake appends `-np` to the firmware basename for this direct-QSPI variant.

The linker reserves a dedicated 40 KiB `RAM_4_EXT` region; in the current layout it is the primary swap/cache area for `VGA256`. M1 can additionally use its external SPI PSRAM paging backend. SD-backed paging uses `286/pagefile.sys`.
...
`NO_PAGING` is currently one of these CMake-level switches. It defaults to `ON`; set `-DNO_PAGING=OFF` explicitly when a paging build is required.
