...
```bat
build.bat -M1 -VGA256 -504 -p 66 --clean
```
...
With five boards and two EMM states this gives 10 combinations per firmware class, or **20 builds** total. Audio no longer multiplies the matrix because it is selected at runtime.

Each variant gets a separate directory below:

```text
build/all/<board>-286-<video>/
build/all/<board>-286-<video>-emm/
```
...
```text
bin/Release/m1p2-286-VGA128-504MHz-1.6V-P66-v1.14-np.uf2
```
...
Last directory contains `config.ini`, first - disk/bios images, third (when SD-backed paging is active) - `pagefile.sys`.
