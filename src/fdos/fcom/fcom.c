...
#define FCOM_LOCAL_STATIC_ASSERT_MACRO 1
#endif
extern "C" {
#include "fdos/hdrs.h"
#include "bios/bios.h"
...
#ifdef FCOM_LOCAL_STATIC_ASSERT_MACRO
#undef _Static_assert
#undef FCOM_LOCAL_STATIC_ASSERT_MACRO
#endif
#undef strchr
#undef new
#ifdef load
...
static inline int fcom_ascii_isalpha(int c)
{
  unsigned char u = (unsigned char)c;
  return (u >= 'A' && u <= 'Z') || (u >= 'a' && u <= 'z');
}

static inline int fcom_ascii_isdigit(int c)
{
  unsigned char u = (unsigned char)c;
  return u >= '0' && u <= '9';
}

static inline int fcom_ascii_isalnum(int c)
{
  return fcom_ascii_isalpha(c) || fcom_ascii_isdigit(c);
}

static inline const char *fcom_strrchr(const char *s, int c)
{
...
static int fcom_chk_cbreak(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g, int mode);
static void build_tail(const fcom_guest_ref &g, const char *args);
static int exec_once(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g,
                     UBYTE exec_mode);
static int execute_command_line(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g, char *line);
...
#define FCOM_DATA_END          (FCOM_WORK_OFFSET + sizeof(struct fcom_guest_layout))
...
using fdos_guest::linear_t;
using fdos_guest::scalar_proxy;
...
public:
...
#else
  fcom_buffer_ref(fcom_buffer_ref &&) = default;
  ~fcom_buffer_ref() = default;
#endif
  ...
  __attribute__((always_inline)) T *operator&() { return static_cast<T *>(*this); }
...
template <typename T>
class fcom_object_ref {
public:
  __attribute__((always_inline)) explicit fcom_object_ref(linear_t addr)
      : bytes_(addr) {}
  __attribute__((always_inline)) T *operator->() { return static_cast<T *>(bytes_); }
  __attribute__((always_inline)) const T *operator->() const { return static_cast<const T *>(bytes_); }
  __attribute__((always_inline)) T *operator&() { return static_cast<T *>(bytes_); }
  __attribute__((always_inline)) const T *operator&() const { return static_cast<const T *>(bytes_); }
private:
  fcom_buffer_ref<T, 1> bytes_;
};

class fcom_guest_ref {
public:
  __attribute__((always_inline)) fcom_guest_ref() : base_(0) {}
  __attribute__((always_inline)) explicit fcom_guest_ref(linear_t base) : base_(base) {}

#define FCOM_SCALAR(type, name) \
  __attribute__((always_inline)) scalar_proxy<type> name() const { \
    return scalar_proxy<type>(base_ + offsetof(struct fcom_guest_layout, name)); \
  }
#define FCOM_BUFFER(type, name) \
  __attribute__((always_inline)) auto name() const { \
    return fcom_buffer_ref<type, sizeof(((struct fcom_guest_layout *)0)->name) / sizeof(type)>( \
        base_ + offsetof(struct fcom_guest_layout, name)); \
  }
#define FCOM_OBJECT(type, name) \
  __attribute__((always_inline)) auto name() const { \
    return fcom_object_ref<type>(base_ + offsetof(struct fcom_guest_layout, name)); \
  }

  FCOM_OBJECT(keyboard, input)
  FCOM_OBJECT(exec_blk, exec_block)
  FCOM_OBJECT(CommandTail, tail)
  FCOM_OBJECT(fcb, exec_fcb1)
  FCOM_OBJECT(fcb, exec_fcb2)
  FCOM_BUFFER(char, filename)
  FCOM_BUFFER(char, program)
  FCOM_BUFFER(char, init_tail)
  FCOM_BUFFER(char, text)
  FCOM_BUFFER(char, path)
  FCOM_BUFFER(char, path2)
  FCOM_OBJECT(dmatch, find)
  FCOM_BUFFER(UBYTE, io)
  FCOM_SCALAR(UWORD, owned_env_seg)
  FCOM_SCALAR(UWORD, owned_env_bytes)
  FCOM_SCALAR(UBYTE, persistent)
  FCOM_SCALAR(UBYTE, skip_autoexec)
  FCOM_BUFFER(char, autoexec_path)
  FCOM_BUFFER(char, comspec_arg)
  FCOM_BUFFER(char, batch_line)
  FCOM_BUFFER(char, batch_name)
  FCOM_BUFFER(char, batch_args)
  FCOM_BUFFER(char, batch_arg)
  FCOM_BUFFER(char, batch_goto)
  FCOM_BUFFER(char, if_left)
  FCOM_BUFFER(char, if_right)
  FCOM_BUFFER(char, for_item)
  FCOM_BUFFER(char, for_command)
  FCOM_BUFFER(char, redirect_command)
  FCOM_BUFFER(char, redirect_in)
  FCOM_BUFFER(char, redirect_out)
  FCOM_BUFFER(char, pipe_left)
  FCOM_BUFFER(char, pipe_right)
  FCOM_BUFFER(char, pipe_temp)
  FCOM_SCALAR(UBYTE, pipe_depth)
  FCOM_SCALAR(UBYTE, batch_active)
  FCOM_SCALAR(UBYTE, cbreak_leave_all)
  FCOM_SCALAR(UBYTE, echo_enabled)
  FCOM_SCALAR(UBYTE, exit_requested)
  FCOM_SCALAR(UBYTE, exit_batch_only)
  FCOM_SCALAR(UWORD, exit_code)
  FCOM_SCALAR(UBYTE, errorlevel)
  FCOM_SCALAR(UBYTE, exit_reason)
  FCOM_SCALAR(UWORD, dir_stack_used)
  FCOM_SCALAR(UWORD, alias_used)
  FCOM_SCALAR(UWORD, history_used)
  FCOM_SCALAR(UBYTE, lfnfor_enabled)
  FCOM_SCALAR(UBYTE, lfncomplete_enabled)
  FCOM_SCALAR(UBYTE, fddebug_enabled)
  FCOM_SCALAR(UBYTE, trace_mode)
  FCOM_SCALAR(UBYTE, batch_depth)
  FCOM_SCALAR(UBYTE, batch_shiftlevel)
  FCOM_SCALAR(UWORD, active_saved_stdin)
  FCOM_SCALAR(UWORD, active_saved_stdout)
  FCOM_SCALAR(UWORD, saved_parent_psp)
  FCOM_OBJECT(intvec, saved_terminate_addr)
  FCOM_SCALAR(UWORD, fddebug_handle)
  FCOM_BUFFER(char, fddebug_name)
...
static inline __attribute__((always_inline)) fcom_guest_ref fcom_state(UWORD command_psp)
{
  return fcom_guest_ref(fcom_guest_linear(command_psp, FCOM_WORK_OFFSET));
}

static inline __attribute__((always_inline)) uint32_t fcom_dir_stack_addr(UWORD command_psp)
{
  return fcom_guest_linear(command_psp, FCOM_DIR_STACK_OFFSET);
}

static inline __attribute__((always_inline)) uint32_t fcom_alias_addr(UWORD command_psp)
{
  return fcom_guest_linear(command_psp, FCOM_ALIAS_OFFSET);
}

static inline __attribute__((always_inline)) uint32_t fcom_history_addr(UWORD command_psp)
{
  return fcom_guest_linear(command_psp, FCOM_HISTORY_OFFSET);
}

static inline __attribute__((always_inline)) uint32_t fcom_loadfix_addr(UWORD command_psp)
{
  return fcom_guest_linear(command_psp, FCOM_LOADFIX_OFFSET);
}
...
static void dos_puts(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g,
                     const char *s)
{
  size_t n = strlen(s);
  if (n >= decltype(g.text())::size())
    n = decltype(g.text())::size() - 1;
  memcpy(g.text(), s, n);
  g.text()[n++] = '$';
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, text);
  CPU_AH = 0x09;
  fcom_intcall(cpu, command_psp, 0x21, "FCOM output");
}

static unsigned fcom_readline(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g)
{
  g.input()->kb_size = FCOM_LINE_MAX;
  g.input()->kb_count = 0;
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, input);
  CPU_AH = 0x0a;
  ...
  {
    auto input = g.input();
    if (input->kb_count > FCOM_LINE_MAX)
      input->kb_count = FCOM_LINE_MAX;
    input->kb_buf[input->kb_count] = '\0';
    return input->kb_count;
  }
}
...
{
  return fcom_ascii_isalpha((unsigned char)name[0]) &&
         name[1] == ':' &&
         name[2] == '\0';
}
...
static int dos_change_dir(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, const char *path)
{
  size_t n = strlen(path);

  if (n >= decltype(g.path())::size())
    return -3;

  memcpy(g.path(), path, n + 1);
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, path);
  CPU_AH = 0x3b;
...
static int dos_get_cwd(CPU *cpu, UWORD command_psp,
                       const fcom_guest_ref &g, unsigned drive)
{
  memset(g.path(), 0, decltype(g.path())::size());
  SET_DS(command_psp);
  CPU_SI = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, path);
  CPU_DL = (UBYTE)drive;
  CPU_AH = 0x47;
                               ...
                               char **dst, size_t *left)
{
  ...
  n = snprintf(g.path(), decltype(g.path())::size(), "%02u-%02u-%04u",
                               ...
                               char **dst, size_t *left)
{
  ...
  n = snprintf(g.path(), decltype(g.path())::size(), "%02u:%02u:%02u.%02u",
...
static void show_prompt(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g)
{
                         ...
                         const fcom_guest_ref &g)
{
  ...
  g.io()[0] = 0x0c;
  (void)fcom_write(cpu, command_psp,
      FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io), 1);
  ...
  if ((attributes & 0x009fu) == 0x0093u) {
    ...
    g.io()[3] = 'J';
    (void)fcom_write(cpu, command_psp,
        FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io), 4);
  }
}

static void pause_command(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, const char *args)
{
...
static void error_bad_command(CPU *cpu, UWORD command_psp,
                              const fcom_guest_ref &g, const char *name)
{
  size_t n = strlen(name);

  if (n >= decltype(g.path())::size())
    n = decltype(g.path())::size() - 1;
...
{
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, find);
  CPU_AH = 0x1a;
...
static int dos_find_first(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, const char *pattern)
{
...
static int fcom_parse_dir_options(const fcom_guest_ref &g, const char *args,
                                  struct fcom_dir_options *options)
{
  ...
  if (strlen(args) >= decltype(g.redirect_command())::size())
    return 0;

  strcpy(g.redirect_command(), args);
  /* Keep the pageable buffer proxy alive while the parser retains a pointer
   * into its scoped host view.  A temporary g.redirect_command() would be
   * destroyed at the end of the assignment expression. */
  auto redirect_command = g.redirect_command();
  cursor = redirect_command;
                                     ...
                                     unsigned wide_column)
{
  ...
  if (options->bare) {
    n = snprintf(g.text(), decltype(g.text())::size(), "%s\r\n",
                 g.find()->dm_name);
  } else if (options->wide) {
    if (g.find()->dm_attr_fnd & D_DIR)
      n = snprintf(g.text(), decltype(g.text())::size(), "[%-12s]",
                   g.find()->dm_name);
    else
      n = snprintf(g.text(), decltype(g.text())::size(), "%-14s",
                   g.find()->dm_name);
    ...
    n = snprintf(g.text(), decltype(g.text())::size(), "%-13s <DIR>\r\n",
                 g.find()->dm_name);
  } else {
    auto find = g.find();
    n = snprintf(g.text(), decltype(g.text())::size(), "%-13s %10lu\r\n",
                 find->dm_name, (unsigned long)find->dm_size);
  }

  if (n > 0) {
    if ((size_t)n >= decltype(g.text())::size())
      n = (int)decltype(g.text())::size() - 1;
    fcom_write(cpu, command_psp,
               FCOM_WORK_OFFSET +
                 (UWORD)offsetof(struct fcom_guest_layout, text),
               (UWORD)n);
...
static void __hfa_func(builtin_dir)(CPU *cpu, UWORD command_psp,
                        const fcom_guest_ref &g, const char *args)
{
  struct fcom_dir_options options;
...
static int fcom_read(CPU *cpu, UWORD command_psp,
                    const fcom_guest_ref &g, UWORD handle)
{
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io);
  CPU_BX = handle;
  CPU_CX = decltype(g.io())::size();
  CPU_AH = 0x3f;
      ...
      }
      {
        if (i > begin) {
          int written = fcom_write(
              cpu, command_psp,
              FCOM_WORK_OFFSET +
                (UWORD)offsetof(struct fcom_guest_layout, io) + begin,
              (UWORD)(i - begin));
...
static void __hfa_func(builtin_type)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  char *cursor = args;
  char *name;
  unsigned argc = 0;
                                        ...
                                        const char *assignment)
{
  ...
  UWORD old_owned_env = fcom_guest_read16(
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, owned_env_seg));
  ...
  fcom_guest_write16(gbase + (uint32_t)offsetof(struct fcom_guest_layout, owned_env_seg),
                     new_seg);
  fcom_guest_write16(gbase + (uint32_t)offsetof(struct fcom_guest_layout, owned_env_bytes),
                     (UWORD)(paras << 4));
  return 0;
}
...
static int fcom_initialize_environment(CPU *cpu, UWORD command_psp)
{
  ...
  const uint32_t program =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, program);
  const uint32_t assignment =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, text);
  const uint32_t path =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, path);
  const uint32_t path2 =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, path2);
  const uint32_t comspec =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, comspec_arg);
  UWORD env_seg = fcom_guest_psp_environment(command_psp);
  size_t variables_bytes;
  size_t trailer_bytes;
  size_t n;
  ...
  if (environment_layout(command_psp, &variables_bytes, &trailer_bytes,
                         program, sizeof(((struct fcom_guest_layout *)0)->program)) < 0)
  ...
  n = fcom_guest_strnlen(comspec,
                         sizeof(((struct fcom_guest_layout *)0)->comspec_arg));
  if (n != 0 && n < sizeof(((struct fcom_guest_layout *)0)->path2)) {
    fcom_guest_copy(path, comspec, n + 1u);

    SET_DS(command_psp);
    CPU_SI = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, path);
    SET_ES(command_psp);
    CPU_DI = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, path2);
    CPU_AH = 0x60;
    ...
    if (!int21_failed(cpu)) {
      int is_dir = 0;
      size_t path2_len =
          fcom_guest_strnlen(path2, sizeof(((struct fcom_guest_layout *)0)->path2));

      if (path2_len < sizeof(((struct fcom_guest_layout *)0)->path2)) {
        ...
        if (is_dir) {
          ...
          if (path2_len + sizeof("\\COMMAND.COM") <=
              sizeof(((struct fcom_guest_layout *)0)->path2)) {
  ...
  n = fcom_guest_strnlen(program, sizeof(((struct fcom_guest_layout *)0)->program));
  if (n >= sizeof(((struct fcom_guest_layout *)0)->text) - sizeof("COMSPEC="))
    return -1;
  ...
  return replace_environment_variable_guest(
      cpu, command_psp, assignment, sizeof(((struct fcom_guest_layout *)0)->text));
}
...
{
  int count;
  const uint32_t io = fcom_guest_linear(
      command_psp,
      FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io));
  ...
  count = CPU_AX;
  if ((size_t)count >= value_size)
    count = (int)value_size - 1;
  fcom_guest_read(io, value, (size_t)count);
  while (count > 0 &&
         (value[count - 1] == '\r' || value[count - 1] == '\n'))
    --count;
  value[count] = '\0';
  return 1;
}
                            ...
                            char *value, size_t value_size)
{
  ...
  snprintf(g.pipe_temp(), decltype(g.pipe_temp())::size(),
           "FC%04X.$$$", command_psp);
  (void)fcom_set_delete_temp(cpu, command_psp, g);

  command_len = strlen(command);
  if (command_len + 1u + strlen(g.pipe_temp()) >= decltype(g.for_command())::size())
    return 0;
  ...
  if (strlen(g.pipe_temp()) >= decltype(g.path())::size())
    return 0;
  ...
  count = int21_failed(cpu) ? -1 : CPU_AX;
...
static void __hfa_func(builtin_set)(CPU *cpu, UWORD command_psp,
                        const fcom_guest_ref &g, char *args)
{
  ...
  if (opt_info) {
    ...
    int n = snprintf(g.text(), decltype(g.text())::size(),
                     "Size of environment segment: %u bytes; unused: %u\r\n",
                     bytes, bytes > used ? bytes - used : 0u);

    if (n > 0)
      (void)fcom_write(cpu, command_psp,
          FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, text),
          (UWORD)n);
  }
  ...
  if (*assignment == '\0') {
    ...
    while (env != FCOM_GUEST_NULL && left != 0 && fcom_guest_read8(env) != 0) {
      ...
      if (fcom_guest_read8(env) > ' ') {
        ...
        while (done < n) {
          size_t chunk = n - done;
          if (chunk > decltype(g.io())::size())
            chunk = decltype(g.io())::size();
          fcom_guest_read(env + (uint32_t)done, g.io(), chunk);
          if (fcom_write(cpu, command_psp,
                         FCOM_WORK_OFFSET +
                           (UWORD)offsetof(struct fcom_guest_layout, io),
                         (UWORD)chunk) < 0)
            return;
          done += chunk;
        }
  ...
  *equal = '\0';
  ...
  if (opt_prompt) {
    if (!fcom_set_read_prompt(cpu, command_psp, g, value,
  ...
  if (opt_execute) {
    if (!fcom_set_execute(cpu, command_psp, g, value,
                           value_buffer, sizeof(value_buffer)))
      return;
    value = value_buffer;
  }
  ...
  if (strlen(assignment) + 1u + strlen(value) >= decltype(g.text())::size()) {
...
static void __hfa_func(builtin_path)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  ...
  if (strlen(p) + 5u >= decltype(g.text())::size()) {
                           ...
                           const fcom_guest_ref &g, char *args)
{
  ...
  if (strlen(p) + 7u >= decltype(g.text())::size()) {
                                    ...
                                    const char *name)
{
  ...
  if (n == 0 || n >= decltype(g.path())::size() ||
      n >= decltype(g.fddebug_name())::size())
    return 0;
                               ...
                               const char *text)
{
  ...
  n = snprintf(g.text(), decltype(g.text())::size(), "%s%s\r\n",
               prefix != NULL ? prefix : "",
               text != NULL ? text : "");
  if (n <= 0)
    return;
...
static void __hfa_func(builtin_fddebug)(CPU *cpu, UWORD command_psp,
                            const fcom_guest_ref &g, char *args)
{
  char *p = skip_space(args);
  char *end;
...
static void __hfa_func(builtin_verify)(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g, char *args)
{
                         ...
                         size_t destination_size)
{
  ...
  if (n >= decltype(g.path())::size() || destination_size == 0)
    return 0;

  memcpy(g.path(), source, n + 1);
  memset(g.text(), 0, decltype(g.text())::size());

  SET_DS(command_psp);
  CPU_SI = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, path);
  SET_ES(command_psp);
  CPU_DI = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, text);
  CPU_AH = 0x60;
...

  {
    auto text = g.text();
    n = 0;
    while (n < decltype(g.text())::size() && text[n] != '\0')
      ++n;
  }
...
static void __hfa_func(builtin_truename)(CPU *cpu, UWORD command_psp,
                             const fcom_guest_ref &g, char *args)
{
  char *p = skip_space(args);
                       ...
                       int change_drive, const char *function_name)
{
  ...
  has_drive = fcom_ascii_isalpha((unsigned char)args[0]) && args[1] == ':';
...
static void fcom_dir_stack_drop_oldest(UWORD command_psp,
                                       const fcom_guest_ref &g)
{
  const uint32_t storage = fcom_dir_stack_addr(command_psp);
  const UWORD used = g.dir_stack_used();
  size_t oldest;

  if (used == 0)
    return;

  oldest = fcom_guest_strnlen(storage, used) + 1u;
  if (oldest >= used) {
    g.dir_stack_used() = 0;
    fcom_guest_write8(storage, 0);
    return;
  }

  fcom_guest_copy(storage, storage + (uint32_t)oldest, used - oldest);
  g.dir_stack_used() = (UWORD)(used - oldest);
}
...
{
  const uint32_t storage = fcom_dir_stack_addr(command_psp);
  size_t needed = strlen(directory) + 1u;

  if (needed > FCOM_DIR_STACK_BYTES)
    return 0;

  while ((size_t)(UWORD)g.dir_stack_used() + needed > FCOM_DIR_STACK_BYTES)
    fcom_dir_stack_drop_oldest(command_psp, g);

  fcom_guest_write(storage + (uint32_t)(UWORD)g.dir_stack_used(),
                   directory, needed);
  g.dir_stack_used() = (UWORD)((UWORD)g.dir_stack_used() + needed);
  return 1;
}

static UWORD fcom_dir_stack_last_offset(UWORD command_psp,
                                        const fcom_guest_ref &g)
{
  const uint32_t storage = fcom_dir_stack_addr(command_psp);
  const UWORD used = g.dir_stack_used();
  ...
  while (pos < used) {
    size_t n;
    last = (UWORD)pos;
    n = fcom_guest_strnlen(storage + (uint32_t)pos, used - pos);
    pos += n + 1u;
  }
  return last;
}
...
static void fcom_dir_stack_pop_last(UWORD command_psp,
                                    const fcom_guest_ref &g)
{
  UWORD last = fcom_dir_stack_last_offset(command_psp, g);
  if (last == 0xffffu)
    return;
  g.dir_stack_used() = last;
  fcom_guest_write8(fcom_dir_stack_addr(command_psp) + last, 0);
}
...
{
  g.dir_stack_used() = 0;
  fcom_guest_write8(fcom_dir_stack_addr(command_psp), 0);
}

static void __hfa_func(builtin_pushd)(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g, char *args)
{
  int rc;
...
static void __hfa_func(builtin_popd)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  char *p = skip_space(args);
  UWORD offset;
  ...
  offset = fcom_dir_stack_last_offset(command_psp, g);
  if (offset == 0xffffu) {
    dos_puts(cpu, command_psp, g,
  ...
  fcom_guest_read(fcom_dir_stack_addr(command_psp) + offset,
                  path2, decltype(g.path2())::size());
  path2[decltype(g.path2())::size() - 1u] = '\0';

  fcom_dir_stack_pop_last(command_psp, g);
  (void)fcom_cd_dir(cpu, command_psp, g, path2, 1, "CDD");
}

static void __hfa_func(builtin_dirs)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  const uint32_t storage = fcom_dir_stack_addr(command_psp);
  const UWORD used = g.dir_stack_used();
  ...
  while (pos < used) {
    size_t len = fcom_guest_strnlen(storage + (uint32_t)pos, used - pos);
    if (len >= sizeof(directory))
      len = sizeof(directory) - 1u;
    fcom_guest_read(storage + (uint32_t)pos, directory, len);
    directory[len] = '\0';
    dos_puts(cpu, command_psp, g, directory);
    dos_puts(cpu, command_psp, g, "\r\n");
    pos += fcom_guest_strnlen(storage + (uint32_t)pos, used - pos) + 1u;
    ++count;
  }
  ...
  n = snprintf(g.text(), decltype(g.text())::size(),
               "%u directory stack entr%s.\r\n",
               count, count == 1 ? "y" : "ies");
  if (n > 0)
    (void)fcom_write(cpu, command_psp,
        FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, text),
...
static char *__hfa_func(next_argument)(char **cursor)
{
  char *p = skip_space(*cursor);
  char *start;
...
static int __hfa_func(fcom_mkdir)(CPU *cpu, UWORD command_psp,
                     const fcom_guest_ref &g, const char *name)
{
  size_t n = strlen(name);
...
static int __hfa_func(fcom_rmdir)(CPU *cpu, UWORD command_psp,
                     const fcom_guest_ref &g, const char *name)
{
  size_t n = strlen(name);
...
static int __hfa_func(dos_unlink)(CPU *cpu, UWORD command_psp,
                      const fcom_guest_ref &g, const char *name)
{
  size_t n = strlen(name);
...
static int __hfa_func(dos_rename_file)(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g,
                           const char *old_name, const char *new_name)
{
  ...
  if (old_len >= decltype(g.path())::size() || new_len >= decltype(g.path2())::size())
    return -3;
...
static void __hfa_func(fcom_dirfct_error)(CPU *cpu, UWORD command_psp,
                              const fcom_guest_ref &g,
...
static int __hfa_func(fcom_md_rd_bool_option)(const char *arg,
                                  int letter, int *value)
{
  const char *p = arg;
  int enabled = 1;
...
static int __hfa_func(fcom_parse_md_rd)(char *args, int mkdir_command,
                            char **directory,
...
static void __hfa_func(fcom_cut_backslash)(char *path)
{
  size_t n = strlen(path);
...
static int __hfa_func(fcom_get_file_attr)(CPU *cpu, UWORD command_psp,
                              const fcom_guest_ref &g,
...
static int __hfa_func(fcom_recursive_mkdir)(CPU *cpu, UWORD command_psp,
                                const fcom_guest_ref &g,
                                const char *directory)
{
  ...
  if (n == 0 || n >= decltype(g.path2())::size())
    return -3;
  ...
  for (i = start; i <= n; ++i) {
    ...
    if (i == start ||
        (i == start + 1 && [&]() {
          auto path2 = g.path2();
          return path2[start] == '\\' || path2[start] == '/';
        }()))
      continue;
...
static int __hfa_func(fcom_rmdir_confirm)(CPU *cpu, UWORD command_psp,
                              const fcom_guest_ref &g,
                              const char *directory)
{
  int ch;
  ...
  g.io()[2] = '\n';
  (void)fcom_write(cpu, command_psp,
      FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io), 3);

  return ch == 'Y';
}

static uint32_t __hfa_func(fcom_rmdir_slot)(uint32_t storage, unsigned index)
{
  return storage + (uint32_t)index * FCOM_RMDIR_PATH_BYTES;
}

static int __hfa_func(fcom_rmdir_join)(char *dst, size_t dst_size,
                           const char *directory,
                           const char *name)
{
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wmaybe-uninitialized"
static int __hfa_func(fcom_recursive_rmdir)(CPU *cpu, UWORD command_psp,
                                const fcom_guest_ref &g,
                                const char *directory,
                                int quiet)
{
  ...
  UWORD storage_seg = 0;
  uint32_t storage;
  char current[FCOM_RMDIR_PATH_BYTES];
  char next[FCOM_RMDIR_PATH_BYTES];
  unsigned used = 1;
  unsigned scan_index;
  int result = -1;
  ...
  {
    const UWORD paras = (UWORD)(((size_t)FCOM_RMDIR_SLOTS *
                                FCOM_RMDIR_PATH_BYTES + 15u) >> 4);
    int allocated = guest_alloc(cpu, command_psp, paras);

    if (allocated < 0)
      return allocated;
    storage_seg = (UWORD)allocated;
    storage = fcom_guest_linear(storage_seg, 0);
  }

  fcom_guest_fill(storage, 0,
                  (size_t)FCOM_RMDIR_SLOTS * FCOM_RMDIR_PATH_BYTES);
  fcom_guest_write(fcom_rmdir_slot(storage, 0),
  ...
  for (scan_index = 0;
    ...
    fcom_guest_read(fcom_rmdir_slot(storage, scan_index),
                    current, sizeof(current));
    current[sizeof(current) - 1u] = '\0';

    if (!fcom_rmdir_join(g.batch_arg(), decltype(g.batch_arg())::size(),
                         current, "*.*"))
      goto done;
    ...
    while (rc == 0) {
      if ((g.find()->dm_attr_fnd & D_DIR) &&
        ...
        if (!fcom_rmdir_join(next, sizeof(next),
                             current, g.find()->dm_name)) {
          restore_default_dta(cpu, command_psp);
          goto done;
        }
        fcom_guest_write(fcom_rmdir_slot(storage, used),
                         next, strlen(next) + 1u);
        ++used;
      }
    ...
    while (rc == 0) {
      if (!(g.find()->dm_attr_fnd & D_DIR) &&
          fcom_rmdir_join(g.path2(), decltype(g.path2())::size(),
                           current, g.find()->dm_name)) {
  ...
  while (used != 0) {
    fcom_guest_read(fcom_rmdir_slot(storage, --used),
                    current, sizeof(current));
    current[sizeof(current) - 1u] = '\0';
  ...
  if (storage_seg != 0)
    guest_free(cpu, command_psp, storage_seg);
  return result;
}
#pragma GCC diagnostic pop

static void __hfa_func(builtin_mkdir)(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, char *args)
{
  char *directory;
  int recursive;
...
static void __hfa_func(builtin_rmdir)(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, char *args)
{
  char *directory;
  int recursive;
...
static int __hfa_func(make_delete_name)(const fcom_guest_ref &g,
                            const char *pattern, const char *found)
{
  const char *slash1 = fcom_strrchr(pattern, '\\');
  const char *slash2 = fcom_strrchr(pattern, '/');
  ...
  if (slash != NULL) {
    ...
    if (prefix + name_len >= decltype(g.path2())::size())
      return 0;
    memcpy(g.path2(), pattern, prefix);
    memcpy(g.path2() + prefix, found, name_len + 1);
  } else {
    if (strlen(found) >= decltype(g.path2())::size())
      return 0;
...
static int __hfa_func(fcom_del_bool_option)(const char *arg,
                                int letter, int *value)
{
  const char *p = arg;
  int enabled = 1;
...
static int __hfa_func(fcom_del_confirm_all)(CPU *cpu, UWORD command_psp,
                                const fcom_guest_ref &g,
                                const char *directory)
{
  int ch;
...
static int __hfa_func(fcom_del_confirm_file)(CPU *cpu, UWORD command_psp,
                                 const fcom_guest_ref &g,
                                 const char *filename,
                                 int *all)
{
...
static const char *__hfa_func(fcom_copy_basename)(const char *name)
{
  const char *base = name;
  const char *p;
...
static int __hfa_func(fcom_del_is_all_pattern)(const char *pattern)
{
  ...
  dot = fcom_strrchr(name, '.');
  return dot == NULL || strcmp(dot, ".*") == 0;
}

static int __hfa_func(fcom_del_directory_text)(char *dst, size_t dst_size,
                                   const char *pattern)
{
  const char *slash1 = fcom_strrchr(pattern, '\\');
  const char *slash2 = fcom_strrchr(pattern, '/');
  const char *colon = fcom_strrchr(pattern, ':');
...
static void __hfa_func(builtin_del)(CPU *cpu, UWORD command_psp,
                        const fcom_guest_ref &g, char *args)
{
  ...
  if (strlen(args) >= decltype(g.batch_line())::size()) {
  ...
  /* Same lifetime rule as DIR: next_argument() advances a pointer inside
   * the proxy buffer, so the proxy itself must outlive the whole loop. */
  auto batch_line = g.batch_line();
  cursor = batch_line;
  while ((arg = next_argument(&cursor)) != NULL) {
    int rc;
    int matched = 0;
    ...
    if (strlen(arg) >= decltype(g.batch_arg())::size()) {
      dos_puts(cpu, command_psp, g, "Filename too long. - '");
...
#pragma GCC diagnostic pop
      if (fcom_get_file_attr(cpu, command_psp, g,
        ...
        if (n + 4 >= decltype(g.batch_arg())::size()) {
          dos_puts(cpu, command_psp, g, "Filename too long. - '");
          dos_puts(cpu, command_psp, g, arg);
          dos_puts(cpu, command_psp, g, "'\r\n");
          return;
    ...
    if (!options.prompt &&
      ...
      if (!fcom_del_directory_text(g.path2(), decltype(g.path2())::size(),
                                   g.batch_arg()) ||
...
static int __hfa_func(fcom_rename_fill_component)(char *dst, size_t dst_size,
                                      const char *pattern,
                                      const char *source)
{
  size_t out = 0;
...
static int __hfa_func(fcom_rename_apply_mask)(char *dst, size_t dst_size,
                                  const char *source_name,
                                  const char *destination_mask)
{
  const char *source_dot = fcom_strrchr(source_name, '.');
  const char *mask_dot = fcom_strrchr(destination_mask, '.');
  char source_base[13];
  char source_ext[4];
  char mask_base[13];
  char mask_ext[4];
...
static int __hfa_func(fcom_rename_build_path)(char *dst, size_t dst_size,
                                  const char *source_pattern,
                                  const char *name)
{
  const char *slash1 = fcom_strrchr(source_pattern, '\\');
  const char *slash2 = fcom_strrchr(source_pattern, '/');
  const char *colon = fcom_strrchr(source_pattern, ':');
  const char *cut = slash1;
  size_t prefix;
  size_t n = strlen(name);
...
static void __hfa_func(builtin_rename)(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g, char *args)
{
  char *cursor = args;
  ...
  if (strlen(source) >= decltype(g.batch_name())::size() ||
      strlen(destination) >= decltype(g.batch_args())::size()) {
    dos_puts(cpu, command_psp, g, "Filename too long.\r\n");
    return;
  }
  ...
  do {
    if (strcmp(g.find()->dm_name, ".") != 0 &&
      ...
      if (!fcom_rename_apply_mask(
              g.program(), decltype(g.program())::size(),
              g.find()->dm_name, g.batch_args()) ||
          !fcom_rename_build_path(
              g.batch_arg(), decltype(g.batch_arg())::size(),
                              ...
                              const char **tail)
{
  ...
  while (*p != '\0' && count < maximum) {
    ...
    if (!fcom_ascii_isdigit((unsigned char)*p))
      break;

    do {
      value = value * 10u + (unsigned)(*p - '0');
      ++p;
    } while (fcom_ascii_isdigit((unsigned char)*p));

    values[count++] = value;

    p = skip_space((char *)p);
    if (*p == '\0' || fcom_ascii_isalpha((unsigned char)*p))
      break;
                                ...
                                char *line, size_t line_size)
{
  ...
  count = CPU_AX;
  if ((size_t)count >= line_size)
    count = (int)line_size - 1;
  fcom_guest_read(io, line, (size_t)count);
  while (count > 0 &&
         (line[count - 1] == '\r' || line[count - 1] == '\n'))
    --count;
  line[count] = '\0';
  return 1;
}

static void __hfa_func(builtin_date)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  char input_line[256];
  char *p = args;
  ...
  for (;;) {
    if (value == NULL) {
      ...
      if (!fcom_read_stdin_line(cpu, command_psp, g,
                                 input_line, sizeof(input_line)))
        return;
      value = input_line;
    }
...
static void __hfa_func(builtin_time)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  char input_line[256];
  char *p = args;
...
static void __hfa_func(builtin_chcp)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  char *p = skip_space(args);
  unsigned current;
...
static void __hfa_func(builtin_vol)(CPU *cpu, UWORD command_psp,
                        const fcom_guest_ref &g, char *args)
{
  char *p = skip_space(args);
  ...
  if (*p == '\0') {
    ...
    if (strlen(p) != 2u || p[1] != ':' ||
        !fcom_ascii_isalpha((unsigned char)p[0])) {
      dos_puts(cpu, command_psp, g, "Syntax error.\r\n");
      return;
    }
  ...
  fcom_guest_fill(fcom_guest_linear(command_psp,
      FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io)),
      0, sizeof(media));
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io);
  ...
  if (media_ok)
    fcom_guest_read(fcom_guest_linear(command_psp,
        FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io)),
        &media, sizeof(media));
  ...
  if (label_ok) {
    ...
    dos_puts(cpu, command_psp, g, find->dm_name);
    dos_puts(cpu, command_psp, g, "\r\n");
  } else {
                           ...
                           char *result, size_t result_size)
{
  ...
  while (path != NULL && *path != '\0') {
    ...
    if (len != 0 &&
        make_path_candidate(g.text(), decltype(g.text())::size(),
                            dir, len, name) &&
...
static void __hfa_func(builtin_which)(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, char *args)
{
  char *cursor = args;
  char *name;
...
static void __hfa_func(builtin_ctty)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  char *cursor = args;
...
{
  const char *source_dot = fcom_strrchr(source_name, '.');
  const char *mask_dot = fcom_strrchr(mask, '.');
  char source_base[13];
  char source_ext[4];
  char mask_base[13];
  char mask_ext[4];
                                 ...
                                 int destination_is_directory)
{
  ...
  const char *slash1 = fcom_strrchr(destination, '\\');
  const char *slash2 = fcom_strrchr(destination, '/');
  const char *colon = fcom_strrchr(destination, ':');
  const char *cut = slash1;
  char generated[16];
  size_t prefix;
                                ...
                                struct fcom_copy_parse *parse)
{
  ...
  if (*env == '\0' ||
      strlen(env) >= sizeof(((struct fcom_guest_layout *)0)->batch_line)) {
    free(env);
    return;
  }
                           ...
                           unsigned *copied)
{
  ...
  if (!wildcard) {
    ...
    if (strlen(lead->name) >= decltype(g.batch_arg())::size())
      return 0;
  ...
  do {
    ...
    auto source_name_ref = g.batch_arg();
    source_name = source_name_ref;
    base = fcom_copy_basename(source_name);

    if (!fcom_copy_destination(g.path2(), decltype(g.path2())::size(),
                               destination, base,
...
static void __hfa_func(builtin_copy)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *args)
{
  /*
   ...
   * a backing page underneath parse->items.  Paths stay in guest memory.
   */
  ...
  if (strlen(args) >= decltype(g.batch_line())::size() ||
      strlen(args) >= decltype(g.redirect_command())::size()) {
    dos_puts(cpu, command_psp, g, "Invalid parameter.\r\n");
    return;
  }
  ...
  if (parse->groups > 1) {
    ...
    if (strlen(parse->items[destination_index].name) >= decltype(g.program())::size()) {
      free(parse->items);
      dos_puts(cpu, command_psp, g, "Invalid parameter.\r\n");
      return;
    }
...
static int fcom_guest_strcasecmp_host(uint32_t addr, size_t len,
                                      const char *host)
{
  size_t i;
  for (i = 0; i < len && host[i] != '\0'; ++i) {
    int a = toupper((unsigned char)fcom_guest_read8(addr + (uint32_t)i));
    int b = toupper((unsigned char)host[i]);
    if (a != b)
      return a - b;
  }
  if (i != len)
...
static void dos_puts_guest(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g,
                           uint32_t addr, size_t len)
{
  while (len != 0) {
    auto text = g.text();
    size_t chunk = len;
    if (chunk >= decltype(g.text())::size())
      chunk = decltype(g.text())::size() - 1u;
    fcom_guest_read(addr, text, chunk);
    text[chunk] = '\0';
...
{
  return fcom_ascii_isalnum((unsigned char)ch) ||
         ch == '_' || ch == '-' || ch == '$';
}
...
static UWORD fcom_alias_find(UWORD command_psp,
                             ...
                             const char *name)
{
  const uint32_t storage = fcom_alias_addr(command_psp);
  const UWORD used = g.alias_used();
  size_t pos = 0;

  while (pos < used) {
    size_t name_len = fcom_guest_strnlen(storage + (uint32_t)pos, used - pos);
    size_t value_pos = pos + name_len + 1u;
    size_t value_len = value_pos < used
        ? fcom_guest_strnlen(storage + (uint32_t)value_pos, used - value_pos)
        : 0;
    if (fcom_guest_strcasecmp_host(storage + (uint32_t)pos, name_len, name) == 0)
      return (UWORD)pos;
    pos = value_pos + value_len + 1u;
  }
  return 0xffffu;
}
                                 ...
                                 UWORD offset)
{
  const uint32_t storage = fcom_alias_addr(command_psp);
  const UWORD used = g.alias_used();
  size_t name_len = fcom_guest_strnlen(storage + offset, used - offset);
  size_t value_offset = (size_t)offset + name_len + 1u;
  size_t value_len = value_offset < used
      ? fcom_guest_strnlen(storage + (uint32_t)value_offset, used - value_offset)
      : 0;
  size_t entry_len = name_len + 1u + value_len + 1u;
  size_t tail = used - (size_t)offset - entry_len;

  if (tail != 0)
    fcom_guest_copy(storage + offset,
                    storage + offset + (uint32_t)entry_len, tail);
  g.alias_used() = (UWORD)(used - entry_len);
  if ((UWORD)g.alias_used() < FCOM_ALIAS_BYTES)
    fcom_guest_write8(storage + (uint32_t)(UWORD)g.alias_used(), 0);
}
                          ...
                          const char *name, const char *value)
{
  ...
  needed = name_len + 1u + value_len + 1u;
  if ((size_t)(UWORD)g.alias_used() + needed > FCOM_ALIAS_BYTES)
    return 0;

  uint32_t dst = storage + (uint32_t)(UWORD)g.alias_used();
  fcom_guest_write(dst, name, name_len + 1u);
  fcom_guest_write(dst + (uint32_t)name_len + 1u, value, value_len + 1u);
  g.alias_used() = (UWORD)((UWORD)g.alias_used() + needed);
  return 1;
}
...
static unsigned fcom_string_table_count_guest(uint32_t storage, UWORD used)
{
  size_t pos = 0;
  unsigned count = 0;
  while (pos < used) {
...
static void __hfa_func(builtin_alias)(CPU *cpu, UWORD command_psp,
                          const fcom_guest_ref &g, char *args)
{
  const uint32_t storage = fcom_alias_addr(command_psp);
  char *p = skip_space(args);
  ...
  if (*p == '\0') {
    ...
    while (pos < g.alias_used()) {
      size_t name_len = fcom_guest_strnlen(storage + (uint32_t)pos,
                                             (UWORD)g.alias_used() - pos);
      size_t value_pos = pos + name_len + 1u;
      size_t value_len = fcom_guest_strnlen(storage + (uint32_t)value_pos,
                                             (UWORD)g.alias_used() - value_pos);

      dos_puts_guest(cpu, command_psp, g, storage + (uint32_t)pos, name_len);
      dos_puts(cpu, command_psp, g, "=");
      dos_puts_guest(cpu, command_psp, g, storage + (uint32_t)value_pos, value_len);
      dos_puts(cpu, command_psp, g, "\r\n");
...
static void fcom_expand_aliases(UWORD command_psp,
                                ...
                                char *line, size_t line_size)
{
  UWORD used[sizeof(((struct fcom_guest_layout *)0)->io) / sizeof(UWORD)];
  ...
  for (;;) {
    ...
    *end = saved;
    ...
    if ((used_count + 1u) * sizeof(UWORD) > decltype(g.io())::size())
      return;
    used[used_count++] = offset;

    {
      uint32_t storage = fcom_alias_addr(command_psp);
      size_t name_len = fcom_guest_strnlen(storage + offset,
                                           (UWORD)g.alias_used() - offset);
      value_addr = storage + offset + (uint32_t)name_len + 1u;
    }

    prefix_len = (size_t)(command - line);
    value_len = fcom_guest_strnlen(value_addr,
                                   (UWORD)g.alias_used() -
                                   (size_t)(value_addr - fcom_alias_addr(command_psp)));
    rest_len = strlen(end);
    ...
    {
      auto expanded = g.for_command();
      memmove(expanded, line, prefix_len);
      fcom_guest_read(value_addr, expanded + prefix_len, value_len);
      memcpy(expanded + prefix_len + value_len, end, rest_len + 1u);
      memcpy(line, expanded, prefix_len + value_len + rest_len + 1u);
    }
  }
}
...
static void fcom_history_drop_oldest(UWORD command_psp,
                                     const fcom_guest_ref &g)
{
  const uint32_t storage = fcom_history_addr(command_psp);
  const UWORD used = g.history_used();
  size_t oldest;
  if (used == 0)
    return;
  oldest = fcom_guest_strnlen(storage, used) + 1u;
  ...
  fcom_guest_copy(storage, storage + (uint32_t)oldest, used - oldest);
  g.history_used() = (UWORD)(used - oldest);
}

static UWORD fcom_history_last_offset(UWORD command_psp,
                                      const fcom_guest_ref &g)
{
  const uint32_t storage = fcom_history_addr(command_psp);
  const UWORD used = g.history_used();
...
static int fcom_guest_string_equals_host(uint32_t addr, size_t maxlen,
                                         const char *host)
{
  size_t n = fcom_guest_strnlen(addr, maxlen);
  size_t hn = strlen(host);
  if (n != hn)
    return 0;
  for (size_t i = 0; i < n; ++i)
    if (fcom_guest_read8(addr + (uint32_t)i) != (UBYTE)host[i])
      return 0;
  return 1;
}
                             ...
                             const char *line)
{
  ...
  if (length > FCOM_HISTORY_BYTES)
    return;
  UWORD last = fcom_history_last_offset(command_psp, g);
  if (last != 0xffffu &&
      fcom_guest_string_equals_host(storage + last,
                                    (UWORD)g.history_used() - last, p))
    return;
  while ((size_t)(UWORD)g.history_used() + length > FCOM_HISTORY_BYTES)
    fcom_history_drop_oldest(command_psp, g);
  fcom_guest_write(storage + (uint32_t)(UWORD)g.history_used(), p, length);
  g.history_used() = (UWORD)((UWORD)g.history_used() + length);
}

static void __hfa_func(builtin_history)(CPU *cpu, UWORD command_psp,
                            const fcom_guest_ref &g, char *args)
{
  const uint32_t storage = fcom_history_addr(command_psp);
  char *p = skip_space(args);
  ...
  if ((UWORD)g.history_used() == 0) {
    dos_puts(cpu, command_psp, g, "Command line history empty.\r\n");
    return;
  }
  while (pos < (UWORD)g.history_used()) {
    size_t len = fcom_guest_strnlen(storage + (uint32_t)pos,
                                    (UWORD)g.history_used() - pos);
    dos_puts_guest(cpu, command_psp, g, storage + (uint32_t)pos, len);
    dos_puts(cpu, command_psp, g, "\r\n");
    pos += len + 1u;
  }
}
                              ...
                              const char *name, unsigned column)
{
  ...
  if (column == 7)
    ...
    n = snprintf(g.text(), decltype(g.text())::size(), "%-10s", name);
...
static void __hfa_func(builtin_doskey)(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g, char *args)
{
  (void)args;
  dos_puts(cpu, command_psp, g,
...
static void __hfa_func(builtin_memory)(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g, char *args)
{
  ...
  unsigned history_count =
      fcom_string_table_count_guest(
          fcom_history_addr(command_psp), g.history_used());
  unsigned dirstack_count =
      fcom_string_table_count_guest(
          fcom_dir_stack_addr(command_psp), g.dir_stack_used());
...
{
  const uint32_t blocks = fcom_loadfix_addr(command_psp);

  while (count != 0) {
    UWORD mcb_seg = fcom_guest_read16(blocks + (uint32_t)(--count * sizeof(UWORD)));
    if (mcb_seg != 0)
...
static int fcom_loadfix_prepare(UWORD command_psp, unsigned *count)
{
  ...
  *count = 0;
  ...
  for (;;) {
    ...
    fcom_guest_write16(blocks + (uint32_t)(*count * sizeof(UWORD)), mcb_seg);
    ++*count;
  }
}
...
static int __hfa_func(builtin_loadfix)(CPU *cpu, UWORD command_psp,
                           const fcom_guest_ref &g, char *args)
{
...
static char *split_command(const fcom_guest_ref &g, keyboard *input)
{
  auto filename = g.filename();
  char *src = skip_space(input->kb_buf);
  char *dst = filename;
  char *dst_end = dst + decltype(g.filename())::size() - 1;
  int quoted = 0;
...
static void build_tail(const fcom_guest_ref &g, const char *args)
{
  ...
  if (n > sizeof(tail->ctBuffer) - 2)
    n = sizeof(tail->ctBuffer) - 2;

  tail->ctCount = (UBYTE)(n ? n + 1 : 0);
  if (n) {
    tail->ctBuffer[0] = ' ';
    memcpy(tail->ctBuffer + 1, args, n);
  }
  tail->ctBuffer[tail->ctCount] = '\r';
}
                            ...
                            const fcom_guest_ref &g)
{
  UWORD source = FCOM_WORK_OFFSET +
      (UWORD)offsetof(struct fcom_guest_layout, tail.ctBuffer);

  memset(&g.exec_fcb1(), 0, sizeof(fcb));
  memset(&g.exec_fcb2(), 0, sizeof(fcb));

  SET_DS(command_psp);
  CPU_SI = source;
  SET_ES(command_psp);
  CPU_DI = FCOM_WORK_OFFSET +
      (UWORD)offsetof(struct fcom_guest_layout, exec_fcb1);
  CPU_AX = 0x2901;
  fcom_intcall(cpu, command_psp, 0x21, "FCOM parse FCB1");

  SET_ES(command_psp);
  CPU_DI = FCOM_WORK_OFFSET +
      (UWORD)offsetof(struct fcom_guest_layout, exec_fcb2);
  CPU_AX = 0x2901;
  fcom_intcall(cpu, command_psp, 0x21, "FCOM parse FCB2");
}

static void capture_exec_status(CPU *cpu, UWORD command_psp,
                                const fcom_guest_ref &g, int exec_rc)
{
  if (exec_rc < 0) {
                     ...
                     UBYTE exec_mode)
{
  ...
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, filename);
  SET_ES(command_psp);
  CPU_BX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, exec_block);
...
static int exec_program(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g,
                        const char *name, UBYTE exec_mode)
{
  ...
  for (i = first; i < last; ++i) {
    ...
    if (n + e >= decltype(g.filename())::size())
      return -2;
...
static int exec_external(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g,
...
static int lh_parseArgs(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g,
...
static void lh_error(CPU *cpu, UWORD command_psp, const fcom_guest_ref &g,
                     int errcode)
{
  switch (errcode)
  {
...
/* cmd_loadhigh() + lh_lf() */
static int __hfa_func(builtin_loadhigh)(CPU *cpu, UWORD command_psp,
                            const fcom_guest_ref &g, char *args)
{
  ...
  if (rc == LH_OK)
  {
    ...
    if (rc == LH_OK)
    {
      ...
      if (!rc)
      {
        ...
        {
          auto filename = g.filename();
          size_t filename_len = strlen(fnam);
          if (filename_len >= decltype(filename)::size())
            filename_len = decltype(filename)::size() - 1;
          memcpy(filename, fnam, filename_len);
                                ...
                                const fcom_guest_ref &g, const char *name)
{
  ...
  if (has_extension(name) || n + 4 >= decltype(g.text())::size())
    return -2;
                           ...
                           const fcom_guest_ref &g, const char *name)
{
  ...
  while (path && *path) {
    ...
    if (len && make_path_candidate(g.text(), decltype(g.text())::size(),
                                   dir, len, name)) {
...
static int read_batch_byte(CPU *cpu, UWORD command_psp, UWORD handle)
{
  SET_DS(command_psp);
  CPU_DX = FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io);
  CPU_BX = handle;
  ...
  return fcom_guest_read8(
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, io)));
}
...
{
  const uint32_t args =
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, batch_args));
  const uint32_t out =
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, batch_arg));
  uint32_t p = args;
  unsigned logical = index +
      fcom_gread8(command_psp, offsetof(struct fcom_guest_layout, batch_shiftlevel));
  ...
  while (fcom_guest_read8(p) != 0) {
    ...
    if (logical == 0) {
      ...
      if (n >= sizeof(((struct fcom_guest_layout *)0)->batch_arg))
        n = sizeof(((struct fcom_guest_layout *)0)->batch_arg) - 1u;
...
static void batch_shift_args_guest(UWORD command_psp, const char *args)
{
  ...
  fcom_gwrite8(command_psp,
               offsetof(struct fcom_guest_layout, batch_shiftlevel), level);
}
...
{
  const uint32_t line =
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, batch_line));
  const uint32_t name =
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, batch_name));
  const uint32_t args =
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, batch_args));
  const uint32_t out =
      fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, for_command));
  uint32_t src = line;
  uint32_t dst = out;
      ...
      if (code == '0') {
        ...
        unsigned skip =
            fcom_gread8(command_psp,
                        offsetof(struct fcom_guest_layout, batch_shiftlevel));
...
{
  fcom_guest_ref fresh = fcom_state(command_psp);
  auto line = fresh.batch_line();
                           ...
                           const fcom_guest_ref &g, int mode)
{
  ...
  const size_t leave_off = offsetof(struct fcom_guest_layout, cbreak_leave_all);
  const size_t active_off = offsetof(struct fcom_guest_layout, batch_active);
  ...
  switch (mode)
  {
    ...
    case BREAK_BATCHFILE:
      ...
      {
        fcom_guest_ref fresh =
            fcom_state(command_psp);
        dos_puts(cpu, command_psp, fresh, "Terminate batch file '");
        {
          size_t batch_len = fcom_guest_strnlen(
              fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, batch_name)),
              sizeof(((struct fcom_guest_layout *)0)->batch_name));
          if (batch_len != 0)
            (void)fcom_write(cpu, command_psp,
                FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, batch_name),
                (UWORD)batch_len);
          else
      ...
      for (;;) {
        ...
        fcom_guest_write8(fcom_gaddr(command_psp, offsetof(struct fcom_guest_layout, io)),
                '\a');
        (void)fcom_write(cpu, command_psp,
            FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, io), 1);
      }
...
static int execute_batch_handle(CPU *cpu, UWORD command_psp, UWORD handle)
{
  const size_t line_off = offsetof(struct fcom_guest_layout, batch_line);
  const size_t goto_off = offsetof(struct fcom_guest_layout, batch_goto);
  size_t used = 0;
  int skip_lf = 0;
  int searching = 0;

  for (;;) {
    int ch = read_batch_byte(cpu, command_psp, handle);
...
{
  const uint32_t gbase = fcom_guest_linear(command_psp, FCOM_WORK_OFFSET);
  const size_t name_off = offsetof(struct fcom_guest_layout, batch_name);
  const size_t args_off = offsetof(struct fcom_guest_layout, batch_args);
  const size_t goto_off = offsetof(struct fcom_guest_layout, batch_goto);
  UBYTE depth = fcom_guest_read8(
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_depth));
  uint32_t saved_addr;
  ...
  fcom_guest_write8(saved_addr + offsetof(struct fcom_batch_context, active),
          fcom_guest_read8(gbase +
              (uint32_t)offsetof(struct fcom_guest_layout, batch_active)));
  fcom_guest_write8(saved_addr + offsetof(struct fcom_batch_context, exit_batch_only),
          fcom_guest_read8(gbase +
              (uint32_t)offsetof(struct fcom_guest_layout, exit_batch_only)));
  fcom_guest_write8(saved_addr + offsetof(struct fcom_batch_context, shiftlevel),
          fcom_guest_read8(gbase +
              (uint32_t)offsetof(struct fcom_guest_layout, batch_shiftlevel)));

  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_depth),
          (UBYTE)(depth + 1u));

  if (guest_name != FCOM_GUEST_NULL) {
    size_t n = fcom_guest_strnlen(
        guest_name, sizeof(((struct fcom_guest_layout *)0)->batch_name) - 1u);
  ...
  fcom_gstr_copy_from_host(command_psp, args_off,
                           sizeof(((struct fcom_guest_layout *)0)->batch_args),
                           args ? args : "");
  fcom_guest_write8(fcom_gaddr(command_psp, goto_off), 0);
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_active), 1);
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, exit_batch_only), 0);
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_shiftlevel), 0);
                           ...
                           uint32_t saved_addr)
{
  ...
  fcom_guest_copy(fcom_gaddr(command_psp,
                             offsetof(struct fcom_guest_layout, batch_name)),
                  saved_addr + offsetof(struct fcom_batch_context, name),
                  sizeof(((struct fcom_batch_context *)0)->name));
  fcom_guest_copy(fcom_gaddr(command_psp,
                             offsetof(struct fcom_guest_layout, batch_args)),
                  saved_addr + offsetof(struct fcom_batch_context, args),
                  sizeof(((struct fcom_batch_context *)0)->args));
  fcom_guest_copy(fcom_gaddr(command_psp,
                             offsetof(struct fcom_guest_layout, batch_goto)),
                  saved_addr + offsetof(struct fcom_batch_context, goto_label),
                  sizeof(((struct fcom_batch_context *)0)->goto_label));
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_active),
          fcom_guest_read8(saved_addr +
              offsetof(struct fcom_batch_context, active)));
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, exit_batch_only),
          fcom_guest_read8(saved_addr +
              offsetof(struct fcom_batch_context, exit_batch_only)));
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_shiftlevel),
          fcom_guest_read8(saved_addr +
              offsetof(struct fcom_batch_context, shiftlevel)));
  fcom_guest_fill(saved_addr, 0, sizeof(struct fcom_batch_context));
  fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_depth), depth);
}
                                ...
                                const char *args)
{
  ...
  rc = execute_batch_handle(cpu, command_psp, (UWORD)handle);
  fcom_close(cpu, command_psp, (UWORD)handle);

  if (fcom_guest_read8(
          gbase + (uint32_t)offsetof(struct fcom_guest_layout, exit_batch_only)))
    rc = 0;

  fcom_batch_pop(command_psp, depth, saved_addr);

  if (!fcom_guest_read8(
          gbase + (uint32_t)offsetof(struct fcom_guest_layout, batch_active))) {
    fcom_guest_ref fresh =
        fcom_state(command_psp);
...
static int execute_default_autoexec(CPU *cpu, UWORD command_psp)
{
  const UWORD path_off =
      FCOM_WORK_OFFSET + (UWORD)offsetof(struct fcom_guest_layout, autoexec_path);
  uint32_t path = fcom_guest_linear(command_psp, path_off);
  unsigned drive = dos_get_drive(cpu, command_psp);
  int rc;
                      ...
                      const fcom_guest_ref &g, char *args)
{
  ...
  if (next != NULL) {
    ...
    if ((size_t)(end - p) >= decltype(g.if_left())::size()) {
      dos_puts(cpu, command_psp, g, "Line too long.\r\n");
      return 0;
    }

    {
      auto if_left = g.if_left();
      memcpy(if_left, p, (size_t)(end - p));
      if_left[(size_t)(end - p)] = '\0';

      size_t if_left_len = strlen(if_left);
      if (if_left[0] == '"' && if_left_len >= 2 &&
          if_left[if_left_len - 1] == '"') {
        memmove(if_left, if_left + 1, if_left_len);
        if_left[if_left_len - 1] = '\0';
      }
    }
...
static int execute_for_value(CPU *cpu, UWORD command_psp,
                             ...
                             const char *command)
{
  strncpy(g.for_item(), value, decltype(g.for_item())::size() - 1);
  g.for_item()[decltype(g.for_item())::size() - 1] = '\0';

  expand_for_variable(command, variable, g.for_item(),
                      g.for_command(), decltype(g.for_command())::size());
...
static int parse_redirections(const fcom_guest_ref &g, const char *line,
                              int *append_output)
{
  const char *srcp = line;
  auto redirect_command = g.redirect_command();
  auto redirect_in = g.redirect_in();
  auto redirect_out = g.redirect_out();
  char *dst = redirect_command;
  char *end = dst + decltype(g.redirect_command())::size() - 1u;
  int quoted = 0;
  ...
  *append_output = 0;
  ...
  while (*srcp != '\0' && dst < end) {
    ...
    if (!quoted && (*srcp == '<' || *srcp == '>')) {
      ...
      target = output ? static_cast<char *>(redirect_out)
                      : static_cast<char *>(redirect_in);
      after = parse_redirect_name((char *)srcp, target,
                                  output ? decltype(g.redirect_out())::size()
                                         : decltype(g.redirect_in())::size());
      if (target[0] == '\0')
        return -1;
      srcp = after;
      continue;
  ...
  while (dst > static_cast<char *>(redirect_command) &&
         (dst[-1] == ' ' || dst[-1] == '\t'))
    --dst;
  *dst = '\0';
  return 0;
...
static int execute_compact_echo(CPU *cpu, UWORD command_psp,
                                const fcom_guest_ref &g, const char *command)
{
  const char *text;
  char delimiter;
...
int __hfa_func(execute_command_core)(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, char *line)
{
  auto input_ref = g.input();
  keyboard *input = input_ref.operator->();
  char *args;
  char *command;
  int suppress_echo = 0;
  int builtin;
  ...
  if (command != input->kb_buf) {
    n = strlen(command);
    if (n > FCOM_LINE_MAX)
      n = FCOM_LINE_MAX;
    memcpy(input->kb_buf, command, n);
    input->kb_buf[n] = '\0';
    input->kb_count = (UBYTE)n;
  }

  args = split_command(g, input);
  if (g.filename()[0] == '\0')
    return 0;
  ...
  if (command_is(g.filename(), "GOTO")) {
    ...
    *end = '\0';
    ...
    {
      auto batch_goto = g.batch_goto();
      size_t label_len = strlen(args);
      if (label_len >= decltype(batch_goto)::size())
        label_len = decltype(batch_goto)::size() - 1;
      memcpy(batch_goto, args, label_len);
      batch_goto[label_len] = '\0';
    }
    return 0;
  }
...
static int split_pipe_command(const fcom_guest_ref &g, const char *line)
{
  const char *p = line;
  ...
  if (left_len == 0 || right_len == 0 ||
      left_len >= decltype(g.pipe_left())::size() ||
      right_len >= decltype(g.pipe_right())::size())
    return -1;
...
static void build_pipe_temp_name(UWORD command_psp, const fcom_guest_ref &g)
{
  snprintf(g.pipe_temp(), decltype(g.pipe_temp())::size(),
 ...
 * already have a process-owned home in class fcom_guest_ref; use that instead of
...
int prepare_redirections(CPU *cpu, UWORD command_psp,
                         const fcom_guest_ref &g, int append_output)
{
  int saved_stdin = -1;
  int saved_stdout = -1;
                               ...
                               const fcom_guest_ref &g, int rc)
{
  ...
  {
    int n = snprintf(g.path(), decltype(g.path())::size(), "%d", rc);
    if (n > 0)
    ...
    UWORD command_psp, UWORD *command_offset)
{
  ...
  const uint32_t init_base =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, init_tail);
  const uint32_t comspec_base =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, comspec_arg);
  const uint32_t autoexec_base =
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, autoexec_path);
  ...
  while (fcom_guest_read8(p) != 0) {
    ...
    if (c != '/' && c != '-') {
      ...
      if (fcom_guest_read8(comspec_base) == 0 && n != 0) {
        if (n >= sizeof(((struct fcom_guest_layout *)0)->comspec_arg))
          n = sizeof(((struct fcom_guest_layout *)0)->comspec_arg) - 1u;
    ...
    switch (option) {
    case 'P':
      fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, persistent), 1);
      ...
      if (c == ':' || c == '=') {
        ...
        while ((c = fcom_guest_read8(p)) != 0 && c != ' ' && c != '\t')
          ++p;
        n = (size_t)(p - word);
        if (n >= sizeof(((struct fcom_guest_layout *)0)->autoexec_path))
          n = sizeof(((struct fcom_guest_layout *)0)->autoexec_path) - 1u;
    ...
    case 'D':
      fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, skip_autoexec), 1);
      p = fcom_guest_skip_option_argument(p);
      break;
    ...
    case 'K':
      ...
      *command_offset = (UWORD)(FCOM_WORK_OFFSET +
          offsetof(struct fcom_guest_layout, init_tail) + (p - init_base));
      return action;
                           ...
                           UWORD start_command_offset)
{
  ...
  if (fcom_guest_read8(gbase +
          (uint32_t)offsetof(struct fcom_guest_layout, persistent)) &&
      !fcom_guest_read8(gbase +
          (uint32_t)offsetof(struct fcom_guest_layout, skip_autoexec)) &&
      start_action == FCOM_START_NONE) {
    const UWORD autoexec_off =
        FCOM_WORK_OFFSET +
        (UWORD)offsetof(struct fcom_guest_layout, autoexec_path);
    ...
    if (fcom_guest_read8(autoexec) != 0)
      ...
      (void)execute_default_autoexec(cpu, command_psp);
  }
...

  {
    UWORD parent = fcom_guest_read16(
        gbase + (uint32_t)offsetof(struct fcom_guest_layout, saved_parent_psp));
    intvec terminate_addr;

    fcom_guest_read(
        gbase + (uint32_t)offsetof(struct fcom_guest_layout, saved_terminate_addr),
  ...
  return (UBYTE)(fcom_guest_read8(
      gbase + (uint32_t)offsetof(struct fcom_guest_layout, exit_requested))
      ? fcom_guest_read16(
          gbase + (uint32_t)offsetof(struct fcom_guest_layout, exit_code))
      : fcom_guest_read8(
          gbase + (uint32_t)offsetof(struct fcom_guest_layout, errorlevel)));
}
...
UBYTE fcom_process_main(CPU *cpu, UWORD command_psp)
{
  ...
  /* fcom_guest_ref addresses the existing COMMAND block directly. */
  ...
  fcom_guest_fill(gbase, 0, sizeof(struct fcom_guest_layout));
  ...
  word = 0xffffu;
  fcom_guest_write16(gbase + (uint32_t)offsetof(struct fcom_guest_layout, active_saved_stdin),
                     word);
  fcom_guest_write16(gbase + (uint32_t)offsetof(struct fcom_guest_layout, active_saved_stdout),
                     word);
  fcom_guest_write16(gbase + (uint32_t)offsetof(struct fcom_guest_layout, saved_parent_psp),
                     saved_parent_psp);
  fcom_guest_write(gbase + (uint32_t)offsetof(struct fcom_guest_layout, saved_terminate_addr),
                   &saved_terminate_addr, sizeof(saved_terminate_addr));
  word = 1u;
  fcom_guest_write16(gbase + (uint32_t)offsetof(struct fcom_guest_layout, fddebug_handle),
                     word);
  fcom_guest_write(gbase + (uint32_t)offsetof(struct fcom_guest_layout, fddebug_name),
                   "stdout", sizeof("stdout"));
  byte = 1u;
  fcom_guest_write(gbase + (uint32_t)offsetof(struct fcom_guest_layout, echo_enabled),
                   &byte, sizeof(byte));
   ...
   */
  {
    ...
    fcom_guest_read(pspbase + (uint32_t)offsetof(psp, ps_cmd.ctCount),
                    &count, sizeof(count));
    if (count >= sizeof(((struct fcom_guest_layout *)0)->init_tail))
      count = sizeof(((struct fcom_guest_layout *)0)->init_tail) - 1u;
    if (count != 0)
      fcom_guest_copy(gbase + (uint32_t)offsetof(struct fcom_guest_layout, init_tail),
                      pspbase + (uint32_t)offsetof(psp, ps_cmd.ctBuffer),
                      count);
    fcom_guest_write8(gbase + (uint32_t)offsetof(struct fcom_guest_layout, init_tail) +
                (uint32_t)count, 0);
  }
  ...
  dos_printf("FCOM: PSP=%04x parent=%04x block=%u paras (%u bytes) "
             ...
             fcom_guest_read16(gbase +
                 (uint32_t)offsetof(struct fcom_guest_layout, saved_parent_psp)),
             (unsigned)FCOM_PROCESS_PARAS,
  ...
  if (fcom_guest_read16(gbase +
          (uint32_t)offsetof(struct fcom_guest_layout, saved_parent_psp)) == DOS_PSP)
